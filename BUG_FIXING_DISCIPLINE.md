...
| BUG-032 | 2026-07-22 | Flutter frontend lacks UI settings for Telegram Bot configuration and reporting schedules; user cannot configure Telegram token/chat ID or scheduled report time from the app | `settings_page.dart` only has server IP/port config and mock mode toggle; `api_client.dart` has no `getSettings()`/`updateSettings()` methods; no `settings_provider.dart` exists; no Telegram-related UI widgets in the app | Pending Phase 13.3 implementation: add settings API methods to `api_client.dart`; create `settings_provider.dart`; add Telegram & Reporting section to `settings_page.dart` with bot token, chat ID, report time, timezone, enable switch, and test button | Pending | Open |

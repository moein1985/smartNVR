...
| BUG-030 | 2026-07-22 | LLM lacks context for `_table` and `_sensitive` Frigate zone naming conventions; cannot answer business questions like "Who was at Soleymani's desk?" or "Were there security alerts in the warehouse?" | `SQL_RULES` in `frigate_schema.py` has 13 rules with no mention of zone naming conventions; `SAMPLE_QUERIES` has no examples of zone+sub_label synergy queries; `get_frigate_zones()` fetches zone names but does not annotate them with semantic type; fallback schema text does not mention zone conventions | Pending Phase 13.1 implementation: add rules 14-16 about `_table`/`_sensitive` conventions; add 4 sample queries; update `get_frigate_zones()` to annotate zone types; fix hardcoded Frigate URL | Pending | Open |
